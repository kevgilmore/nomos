...
export function NomosToastViewport() {
  ...
  React.useEffect(() => {
    const onToast = (event: Event) => {
      ...
      window.setTimeout(() => setToasts((current) => current.filter((item) => item.id !== toast.id)), detail.duration ?? (detail.tone === "error" ? 6000 : 4000));
    };
  ...
  return <div className="pointer-events-none fixed bottom-4 left-4 z-[100] flex w-[min(22rem,calc(100vw-2rem))] flex-col gap-2" aria-live="polite" aria-atomic="true">
    {toasts.map((toast) => {
      ...
      return <div key={toast.id} role={error ? "alert" : "status"} className={`pointer-events-auto flex items-start gap-2 rounded-xl border px-3 py-2.5 text-sm shadow-xl backdrop-blur ${error ? "border-red-400/35 bg-red-950/95 text-red-50" : "border-emerald-400/35 bg-emerald-950/95 text-emerald-50"}`}>
        ...
        <button type="button" onClick={() => setToasts((current) => current.filter((item) => item.id !== toast.id))} className="-mr-1 rounded p-0.5 opacity-70 hover:opacity-100" aria-label="Dismiss notification"><X className="size-4" /></button>
      </div>;
    })}
  </div>;
}
