...
function siteForHost(hostname: string) {
  if (["fitness.nomos.codes", "nomos-fitness.web.app"].includes(hostname)) return "fitness";
  if (["nomos.codes", "www.nomos.codes", "nomos-home.web.app", "nomos-2aafe.web.app"].includes(hostname)) return "home";
  return null;
}
...
export const authGate = onRequest({ region: "europe-west2", secrets: [approvedEmails], invoker: "public" }, async (req, res) => {
