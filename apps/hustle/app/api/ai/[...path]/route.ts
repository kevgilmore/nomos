import { LOCAL_DEV_USER, LOCAL_SESSION_COOKIES } from "@nomos/auth";
import { handleLocalAiRequest } from "@nomos/ai/local";
import { NextRequest, NextResponse } from "next/server";

async function handle(request: NextRequest) {
  if (!request.cookies.get(LOCAL_SESSION_COOKIES.fitness)?.value) return NextResponse.json({ error: "Authentication required" }, { status: 401 });
